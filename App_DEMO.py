...
class Ui_MainWindow(object):
	def setupUi(self, MainWindow):
		...
		self.label_Slect.setGeometry(QtCore.QRect(130, 10, 320, 30))
		myFont=QtGui.QFont()
				...
				for (rectangle, point_mtcnn) in zip(rectangles, points_mtcnn):
					...
					if self.model_using =="Cosine":
						...
						if temp_score <= recognition_threshold:
							Name_face = self.facesbank_Name[temp_indexs]
						...
						cv2.putText(image_show, str(round(temp_score, 3)), (int(rectangle[0]+30), int(rectangle[1]+30)),
									cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
						self.label_face.setText("Name: " + Name_face.split("-")[0])
						self.score_face.setText("Score: " + str(round(temp_score, 3)))
					elif self.model_using == "SVM":
					# 	features_face = []
					# 	features_face.append([feature_face])
					# 	embs_face = l2_normalize(np.concatenate(features_face))
					# 	# name_face = self.Model_SVM_ClassName[self.Model_SVM.predict(embs_face)[0]]
					# 	class_index = self.Model_SVM.predict(embs_face)[0]
					# 	#print(class_index)
					# 	Name_face = self.Model_SVM_ClassName[self.Model_SVM_ClassCode.index(class_index)]
					# 	#temp_score = my_loss_func(self.Model_SVM.predict(embs_face)[0],self.Model_SVM.predict(embs_face)[0])
					# 	cv2.putText(image_show, Name_face , (int(rectangle[0]), int(rectangle[1])),cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2)
					#
					# self.label_face.setText("Name: " + Name_face.split("-")[0])
					#
					# #self.score_face.setText("Score: " + str(temp_score))
						features_face = []
						features_face.append([feature_face])
						embs_face = l2_normalize(np.concatenate(features_face))
						class_index = self.Model_SVM.predict(embs_face)[0]
						yhat_prob = self.Model_SVM.predict_proba(embs_face)
						class_probability = yhat_prob[0, class_index] * 100
						score = class_probability / 100
						Name_face = self.Model_SVM_ClassName[self.Model_SVM_ClassCode.index(class_index)]
						# text=Name_face+"--"+str(round(float(class_probability / 100), 3))
						if (score < 0.4):
							cv2.putText(image_show, "Strange", (int(rectangle[0]), int(rectangle[1])),
										cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
							cv2.putText(image_show, str(round(float(class_probability / 100), 3)),
										(int(rectangle[0] + 30), int(rectangle[1] + 30)), cv2.FONT_HERSHEY_SIMPLEX, 1,
										(0, 0, 255), 2)
						else:
							cv2.putText(image_show, Name_face, (int(rectangle[0]), int(rectangle[1])),
										cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
							cv2.putText(image_show, str(round(float(class_probability / 100), 3)),
										(int(rectangle[0] + 30), int(rectangle[1] + 30)), cv2.FONT_HERSHEY_SIMPLEX, 1,
										(0, 0, 255), 2)
						self.score_face.setText("Score: " + str(round(float(class_probability / 100), 3)))
						self.label_face.setText("Name: " + Name_face.split("-")[0])
						self.score_face.setText("Score: " + str(round(float(class_probability / 100), 3)))
		...
		if ret_mtcnn is None:
			...
			for (rectangle, point_mtcnn) in zip(rectangles, points_mtcnn):
				...
				if self.model_using =="Cosine":
					...
					cv2.putText(image_show, Name_face, (int(rectangle[2]), int(rectangle[3])), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 0, 0), 1)
				elif self.model_using == "SVM":
				# 	features_face = []
				# 	features_face.append([feature_face])
				# 	embs_face = l2_normalize(np.concatenate(features_face))
				...
				# 	Name_face = self.Model_SVM_ClassName[self.Model_SVM_ClassCode.index(class_index)]
				#
				# 	# Name_face = name_face + " - " +str(round(temp_score, 3))
